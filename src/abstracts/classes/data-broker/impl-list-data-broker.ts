...
import { DATA_COMPARATOR, PAGINATION_OPTIONS } from "../../types/common";
 ...
 */
export abstract class ImplListDataBroker<U,D,S,EV_Type> implements ListDataBroker<U,D,S,EV_Type>{
     ...
     */
    constructor( paginationOptions:PAGINATION_OPTIONS, private idPropName:string , private fetchOneResultAsLatest:boolean = true ){
     ...
     */
    async load( options : ListDataBrokerLoadOptions<S> ): Promise<ListDataBrokerResult<D[]>> {

        const nextPage = this.paginatedDataManager.nextPage;
        const page = options.page;

        const forSearch = !!options.search;

        // if the load call is not for search
        if(this.paginatedDataManager.batchExists(page) && !forSearch && options.checkCache ){
     ...
     */
    abstract fetch(options: ListDataBrokerLoadOptions<S>): Promise<ListDataBrokerResult<D[]>>;
}
